...
export default async function MemorialDetailPage({ params }: Props) {
    const memorial = await prisma.memorial.findUnique({
        ...
        include: {
            photos: true,
            // Include offerings and guestbook (recent/all depending on volume, limit for initial load?)
            // For now, fetching all might be ok if volume is low, but better to limit if many.
            // Let's take recent 50 offerings and guestbook entries for initial render.
            offerings: {
